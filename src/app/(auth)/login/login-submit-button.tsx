"use client";

import { Loader2 } from "lucide-react";
import { useFormStatus } from "react-dom";

export function LoginSubmitButton() {
  const { pending } = useFormStatus();

  return (
    <button
      type="submit"
      disabled={pending}
      className="border-input flex h-12 w-full items-center justify-center gap-3 rounded-xl border bg-white px-4 text-sm font-semibold text-[#0a1b33] shadow-sm transition-[background-color,border-color,box-shadow,transform] hover:border-[#939db2] hover:bg-[#f8faff] hover:shadow-md active:translate-y-px disabled:cursor-wait disabled:opacity-70"
    >
      {pending ? (
        <Loader2
          className="size-5 animate-spin text-[#4285F4]"
          aria-hidden="true"
        />
      ) : (
        <GoogleMark />
      )}
      {pending ? "Opening Google…" : "Continue with Google"}
    </button>
  );
}

function GoogleMark() {
  return (
    <svg className="size-5 shrink-0" viewBox="0 0 18 18" aria-hidden="true">
      <path
        fill="#4285F4"
        d="M17.64 9.2c0-.64-.06-1.25-.16-1.84H9v3.48h4.84a4.14 4.14 0 0 1-1.8 2.72v2.26h2.92c1.71-1.57 2.68-3.89 2.68-6.62Z"
      />
      <path
        fill="#34A853"
        d="M9 18c2.43 0 4.47-.8 5.96-2.18l-2.92-2.26c-.81.54-1.84.86-3.04.86-2.34 0-4.32-1.58-5.03-3.7H.96v2.34A9 9 0 0 0 9 18Z"
      />
      <path
        fill="#FBBC05"
        d="M3.97 10.72a5.4 5.4 0 0 1 0-3.44V4.94H.96a9 9 0 0 0 0 8.12l3.01-2.34Z"
      />
      <path
        fill="#EA4335"
        d="M9 3.58c1.32 0 2.5.45 3.44 1.35l2.58-2.58C13.46.89 11.43 0 9 0A9 9 0 0 0 .96 4.94l3.01 2.34C4.68 5.16 6.66 3.58 9 3.58Z"
      />
    </svg>
  );
}
