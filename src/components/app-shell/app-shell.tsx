    ...
    <div className="flex min-h-full flex-1">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <aside className="bg-sidebar absolute inset-y-0 left-0 flex w-64 flex-col border-r shadow-raised">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="bg-card/80 sticky top-0 z-30 flex h-14 shrink-0 items-center justify-between gap-3 border-b px-3 backdrop-blur sm:px-4">
          <button
            type="button"
            aria-label="Open navigation"
            onClick={() => setMobileOpen(true)}
            className="hover:bg-secondary text-muted-foreground flex size-9 items-center justify-center rounded-md lg:hidden"
          >
            <Menu className="size-5" />
          </button>

          <span className="text-sm font-medium lg:hidden">Inforvio PM</span>
...
function Brand() {
  return (
    <div className="flex h-14 items-center gap-2.5 px-4">
      <span className="bg-primary text-primary-foreground flex size-7 shrink-0 items-center justify-center rounded-md text-[11px] font-bold">
        PM
      </span>
...
function SidebarNav({
  ...
  return (
    <nav className="flex-1 overflow-y-auto p-3">
      ...
      {isAdmin && (
        <>
          <p className="text-muted-foreground px-3 pt-6 pb-2 text-[11px] font-semibold tracking-wider uppercase">
            Administration
          </p>
          <ul className="space-y-0.5">
            {ADMIN_NAV.map((item) => (
...
function NavLink({
  ...
  return (
    ...
    >
      ...
      {active && (
        <span
          className="bg-accent absolute top-1.5 bottom-1.5 -left-0.5 w-0.5 rounded-full"
          aria-hidden="true"
        />
      )}
  ...
  */
  return (
    <button
      ...
      className="hover:bg-secondary text-muted-foreground hover:text-foreground flex size-9 items-center justify-center rounded-md transition-colors"
    >
...
function UserMenu({ user }: { user: ShellUser }) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="hover:bg-secondary flex items-center gap-2 rounded-md p-1 pr-2 transition-colors focus-visible:outline-none">
